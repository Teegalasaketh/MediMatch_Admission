...
│  │  Signup      │   │   (Main UI)  │   │                            │   │
│  │              │   │              │   │  ┌──────────────────────┐  │   │
│  │  Supabase    │   │  FilterForm  │   │  │ CollegeDetailModal   │  │   │
│  │  Auth SDK    │   │  CollegeCard │   │  │ (ChatGPT streaming)  │  │   │
│  └──────┬───────┘   └──────┬───────┘   │  └──────────────────────┘  │   │
│         │                  │           │  ┌──────────────────────┐  │   │
│         │                  │           │  │ CollegeAnalytics     │  │   │
│         ▼                  │           │  │ Modal (5 Recharts)   │  │   │
...
│                        BACKEND  (FastAPI)                                 │
│                                                                           │
│  POST /recommendations            POST /chat/stream                       │
│  ┌────────────────────┐           ┌───────────────────────────────────┐  │
│  │  1. Filter Layer   │           │  Server-Sent Events (SSE)         │  │
│  │  ─────────────     │           │  ┌─────────────────────────────┐  │  │
│  │  Exam / Course /   │           │  │  Groq API  (stream=True)    │  │  │
│  │  Budget / State /  │           │  │  llama-3.3-70b-versatile    │  │  │
│  │  College Type      │           │  │                             │  │  │
│  │                    │           │  │  Token → chunk → SSE line   │  │  │
│  │  2. Scoring Engine │           │  │  → Frontend appends text    │  │  │
│  │  ─────────────     │           │  └─────────────────────────────┘  │  │
│  │  Rank    40%       │           └───────────────────────────────────┘  │
│  │  NIRF    20%       │                                                   │
│  │  Placement 20%     │  GET /meta        GET /health                     │
│  │  Budget  20%       │                                                   │
│  │                    │                                                   │
│  │  3. RAG Retrieval  │                                                   │
│  │  ─────────────     │                                                   │
...
│  └────────────────────┘                                                   │
│                                                                           │
│  College Dataset (300+ colleges, in-memory Python dataclasses)           │
│  IITs · NITs · IIITs · Private Engineering · Medical · Central Univ.     │
└─────────────────────────────────────────────────────────────────────────┘
```
...
│                                                                        │
│  StudentPreferences                                                    │
│  { exam, rank, budgetMax, state, course, collegeType, useAI }         │
│                   │                                                    │
│                   ▼                                                    │
│         ┌─────────────────┐                                           │
│         │  FILTER LAYER   │  ── Removes ineligible colleges           │
│         │                 │     (wrong exam/course/budget/state)      │
│         └────────┬────────┘                                           │
│                  │  filtered[]                                         │
│                  ▼                                                     │
│         ┌─────────────────┐                                           │
│         │ SCORING ENGINE  │  ── Weighted formula → matchScore 0-100   │
│         │                 │     Rank(40) NIRF(20) Place(20) Budget(20)│
│         └────────┬────────┘                                           │
│                  │  scored[], sorted desc                              │
│                  ▼                                                     │
│         ┌─────────────────┐                                           │
...
│    │                         │                                        │
│    │  ┌───────────────────┐  │                                        │
│    │  │  RAG RETRIEVAL    │  │  ── Embed student query (TF-IDF)       │
│    │  │  VectorStore      │  │     Cosine similarity search           │
│    │  │  .query(k=3)      │  │     → Top-3 similar colleges as context│
│    │  └─────────┬─────────┘  │                                        │
│    │            │ rag_context │                                        │
│    │            ▼             │                                        │
│    │  ┌───────────────────┐  │                                        │
│    │  │  GROQ LLM CALL    │  │  ── System: counselor persona          │
│    │  │  (if useAI=true)  │  │     User: college + student + RAG ctx  │
│    │  │  llama-3.3-70b    │  │     → 2-3 sentence explanation         │
│    │  └─────────┬─────────┘  │                                        │
│    │            │ explanation │                                        │
│    │            ▼             │                                        │
│    │  ┌───────────────────┐  │                                        │
│    │  │  CLASSIFY CHANCE  │  │  ── rank/cutoff ratio                  │
│    │  │  Safe/Target/Dream│  │     ≤0.7 → Safe                        │
│    │  └───────────────────┘  │     ≤1.0 → Target                      │
│    └─────────────────────────┘     >1.0 → Dream                       │
│                  │                                                     │
│                  ▼                                                     │
│         RecommendationResponse                                         │
│         { total_filtered, results[], student_summary }                │
└──────────────────────────────────────────────────────────────────────┘
     │
     ▼
Frontend renders CollegeCard list
     │
     ├── Click "Analyze" ──► CollegeAnalyticsModal
     │                        (5 Recharts graphs, no API call)
     │
     └── Click "Ask AI" ──► CollegeDetailModal
                             │
                             └── POST /chat/stream (SSE)
                                  │
                                  └── Groq stream=True
                                       Token-by-token → UI append
```
...
Returns ranked college recommendations for a student profile.
...
{
  "total_filtered": 23,
  "student_summary": "Found 23 eligible colleges. Showing top 10: 3 Safe, 5 Target, 2 Dream.",
  "results": [
    {
      "id": 8,
      ...
      "explanation": "Your rank of 5,000 aligns with IIT Hyderabad's closing rank..."
    }
  ]
}
```
...
Streams an LLM response as **Server-Sent Events** (SSE). Used by the AI chat modal.

**Request Body:**
```json
{
  "college": { "college_name": "IIT Hyderabad", ... },
  "student_rank": 5000,
  "student_budget": 300000,
  "system_prompt": "You are an expert Indian college admission counselor...",
  "user_message": "What are the top recruiters at this college?"
}
```

**Response:** `text/event-stream`
```
data: {"choices": [{"delta": {"content": "IIT"}}]}
data: {"choices": [{"delta": {"content": " Hyderabad"}}]}
data: {"choices": [{"delta": {"content": " has..."}}]}
...
data: [DONE]
```

The frontend reads each chunk and appends `content` to the chat bubble in real-time.
